...
// для пересылки главного элемента
struct Args {
    double elem1;
    int elem2;
};
...
void change_block_lines(double *A, double *B, int begin, int q, int pivot, int m, int lines, double *V2, double *V3, double *vec1, double *vec2) {
	for (int i = begin; i < lines; i++) {
		get_block(A,     q, i, lines * m, m, V2, m, m);
		get_block(A, pivot, i, lines * m, m, V3, m, m);
		set_block(A,     q, i, lines * m, m, V3, m, m);
		set_block(A, pivot, i, lines * m, m, V2, m, m);
	}

	// меняем местами блоки векторов
    for (int i = 0; i < m; i++) {
        vec1[i] = B[q * m + i];
        vec2[i] = B[pivot * m + i];
    }
...
int get_main_block(Args &glavny, double *buf, /* int n, */ int m, int size, /* int lines, */ int p, int q, int step, double *V1, double *V2, double norma) {
    glavny.elem1 = 1e304;
	glavny.elem2 = -1;
	...
	for (int i = step + q; i < size; i += p) {
		...
		if (result == 0) {
			...
			if (minimum < glavny.elem1) {
				glavny.elem1 = minimum;
				glavny.elem2 = i;
			}
		}
	}
...
// подредачить //////////////////////////////////////////////////////////////////////// остальное нормально сделано, но тут можно упростить
void subtract_with_multiply(double *A, int other_line, int pivot_line, int line_length, double *pivot_block, int col_cnt, int m, double *V2, double *V3) {
	for (int w = line_length; w < col_cnt; w++) {
		get_block(A, pivot_line, w, col_cnt * m, m, V2, m, m);
		multiply_blocks(pivot_block, m, m, V2, m, m, V3);
		get_block(A, other_line, w, col_cnt * m, m, V2, m, m);
		for (int i = 0; i < m * m; i++) V2[i] -= V3[i];
		set_block(A, other_line, w, col_cnt * m, m, V2, m, m);
	}
}

void subtract_with_multiply_vector (double *B, int other_line, int pivot_line, double *pivot_block, int m, double *buf_vec_1, double *buf_vec_2) {
    for (int i = 0; i < m; i++) buf_vec_1[i] = B[pivot_line * m + i];
	for (int i = 0; i < m; i++) {
		double s = 0;
        for (int j = 0; j < m; j++) s += pivot_block[i * m + j] * buf_vec_1[j];
        buf_vec_2[i] = s;
    }
	for (int i = 0; i < m; i++) buf_vec_1[i] = B[other_line * m + i];
    for (int i = 0; i < m; i++) buf_vec_1[i] -= buf_vec_2[i];
    for (int i = 0; i < m; i++) B[other_line * m + i] = buf_vec_1[i];
}
...
int solver(double *A, double *B, double *buf, double norma, int n, int m, int k, int lines, int p, int q, MPI_Comm comm) {
	...
	for (int i = 0; i < k; i++) {
		...
		if (owner == q) {				// копируем столбец, чтобы отправить его остальным
			for (int d = 0; d < k * m; d++)	for (int e = 0; e < m; e++) buf[d * m + e] = A[d * lines * m + (i / p) * m + e];
		}
		...
		if (result < 0) {
			...
			delete [] V2;
			delete [] V3;
			if (V1_l != nullptr) delete [] V1_l;
			if (V2_l != nullptr) delete [] V2_l;
			delete [] buf_vec_1;
			delete [] buf_vec_2;
			return result;
		}
